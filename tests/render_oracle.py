...
def inspect_frame(frame, revision, phase, output, evidence):
    ...
    for descriptor,data in ((attachments["color"],color),(attachments["object_id"],ids),(attachments["depth"],depth)):
        evidence.retain("readbacks/"+descriptor["path"],data)
    # The output is synthesized from strictly checked fields, never arbitrary vendor logs.
    evidence.retain_json("frames/"+phase+".json",{"frame":frame,"pixel_assertions":stats})
    return stats
...
def inspect_lifecycle(report):
    ...
    require([event["kind"] for event in events] == ["resized","minimized","restored"],"actual resize/minimize/restore event order")
    require(not events[0]["minimized"] and events[1]["minimized"] and not events[2]["minimized"],
            "actual minimized state and recovery observed")
    require(events[1]["submission_serial"] == events[2]["submission_serial"],
    ...
    require(events[0]["pixel_extent"] == frames[2]["pixel_extent"],"resize event matches new framebuffer")
    require(events[1]["submission_serial"] >= frames[2]["submission_serial"] and
            frames[3]["submission_serial"] > events[2]["submission_serial"],
    ...
    for entry in trace:
        ...
        if event == "submit":
            ...
            require(serial in submitted and serial not in graphics_done,"graphics completion follows one actual submission")
            graphics_done.add(serial)
        elif event == "present_complete":
            require(serial in queued and serial not in present_done,"presentation completion follows a queued presentation")
            ...
            require(bool(owned) and owned <= graphics_done and owned <= present_done,
            ...
            queued_resize |= (serial == frames[1]["submission_serial"] and
