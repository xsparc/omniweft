...
"""Prove the independent oracle catches a real atomic-rollback mutation."""
import argparse
import difflib
import hashlib
import json
...
TARGET = "src/transactions.cpp"
ORIGINAL = '    // Rejection discards every staged write and provisional identity binding.'
MUTANT = '    std::swap(target_world.state_, staged);'
...
class Evidence(audit.Evidence):
    def __init__(self, executable, directory):
        ...
        self.manifest.update(
            work_item="PR-003", example="objects.atomic",
            proof="deliberate_atomic_rollback_mutation",
            limitations=["A temporary source copy deliberately publishes staged state on rejection. Passing means the independent full-state oracle detects that defect; it does not validate the mutated executable for use."])
...
def main():
    ...
    with Evidence(baseline_executable, args.evidence_dir) as evidence, tempfile.TemporaryDirectory(prefix="ow-mutant-") as temp:
        ...
        audit.equal("rollback-discard statement appears exactly once", text.count(ORIGINAL), 1)
        ...
        evidence.retain("original-transactions.cpp", before)
        evidence.retain("mutated-transactions.cpp", after)
        ...
        evidence.manifest["mutation"] = {
            ...
            "expected_oracle_failure": "staged-prefix-rollback: full-state after"}
        ...
        oracle = run([sys.executable, str(ROOT / "tests/objects_oracle.py"),
                      "--executable", str(mutated_executable), "--case-set", "rollback"], timeout=60)
        audit.equal("independent oracle rejects deliberate mutant", oracle.returncode, 1)
        audit.require("staged-prefix-rollback: full-state after" in oracle.stderr,
                      "oracle fails specifically because rejected work leaked authoritative state")
