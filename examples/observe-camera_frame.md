# observe.camera_frame

Status: **planned specification; not implemented**. Work item: **PR-029 — Camera, depth and object observations**.

Dependencies: PR-010, PR-018, PR-024. Validation lanes: cpu, gpu.
...
```sh
omniweft_examples --example observe.camera_frame --gpu --seed 7 --verify --output artifacts/observe.camera_frame
```
...
Read a bounded color/depth/object-ID region with view, frame, camera, tick and revision metadata; known picking hits match world IDs.

## Negative and recovery cases

Stale-frame selections and denied/oversized observations fail without blocking rendering.
