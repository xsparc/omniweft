...
Status: **planned specification; not implemented**. Work item: **PR-019 — Versioned mesh vertex edits**.

Dependencies: PR-017. Validation lanes: cpu.
...
```sh
omniweft_examples --example meshes.deform --headless --seed 7 --verify --output artifacts/meshes.deform
```

## Pass criteria

Edit a selected vertex set and verify exact intended positions, recomputed bounds and unchanged untargeted data.

## Negative and recovery cases

Stale topology revision, NaN and out-of-range elements reject the whole edit.
