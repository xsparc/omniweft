# interaction.pick_and_move

Status: **planned specification; not implemented**. Work item: **PR-034 — Human interaction through command API**.

Dependencies: PR-004, PR-009, PR-025, PR-029. Validation lanes: cpu, gpu, manual.
...
```sh
omniweft_examples --example interaction.pick_and_move --gpu --seed 7 --verify --output artifacts/interaction.pick_and_move
```
...
Pick/focus/move an object with documented input controls; user and AI actions use the same revision and authority rules.

## Negative and recovery cases

Competing edits conflict clearly; input focus and remapping prevent accidental world actions while typing.
