# quality.hostile_session

Status: **planned specification; not implemented**. Work item: **PR-036 — Adversarial operation and recovery harness**.

Dependencies: PR-014, PR-015, PR-020, PR-021, PR-023, PR-028, PR-032. Validation lanes: cpu.
...
```sh
omniweft_examples --example quality.hostile_session --headless --seed 7 --verify --output artifacts/quality.hostile_session
```

## Pass criteria

Seeded malformed sessions, allocation failures and restart injection preserve state, quotas and authority invariants under supported sanitizers.

## Negative and recovery cases

Retain minimized failing seeds and detect deliberate regression mutations in validators and commit paths.
