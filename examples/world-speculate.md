...
Status: **planned specification; not implemented**. Work item: **PR-031 — Isolated speculative world branches**.

Dependencies: PR-014, PR-016, PR-028. Validation lanes: cpu.
...
```sh
omniweft_examples --example world.speculate --headless --seed 7 --verify --output artifacts/world.speculate
```

## Pass criteria

Edit and save a copy-on-write branch with its own world identity while the live world remains unchanged.

## Negative and recovery cases

Branch quota/cancel failures reclaim resources without collecting assets retained by live state or history.
