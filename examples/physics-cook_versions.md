# physics.cook_versions

Status: **planned specification; not implemented**. Work item: **PR-027 — Versioned asynchronous collider cooking**.

Dependencies: PR-017, PR-020, PR-024, PR-025. Validation lanes: cpu.
...
```sh
omniweft_examples --example physics.cook_versions --headless --seed 7 --verify --output artifacts/physics.cook_versions
```

## Pass criteria

Cook supported static/convex geometry off-thread and stage the matching revision for publication.

## Negative and recovery cases

A superseded or failed cook never replaces the current collider; staged allocations are reclaimed.
