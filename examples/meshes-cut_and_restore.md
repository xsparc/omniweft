# meshes.cut_and_restore

Status: **planned specification; not implemented**. Work item: **PR-020 — Bounded mesh topology edits**.

Dependencies: PR-012, PR-019. Validation lanes: cpu.
...
```sh
omniweft_examples --example meshes.cut_and_restore --headless --seed 7 --verify --output artifacts/meshes.cut_and_restore
```

## Pass criteria

Add/remove a bounded face selection with valid indices and an explicit element remap or invalidation; undo restores source data.

## Negative and recovery cases

Invalid topology under the supported policy and stale element IDs cannot create corrupt assets.
