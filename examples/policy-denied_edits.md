...
Status: **planned specification; not implemented**. Work item: **PR-007 — Capabilities and resource budgets**.
...
```sh
omniweft_examples --example policy.denied_edits --headless --seed 7 --verify --output artifacts/policy.denied_edits
```

## Pass criteria

Permit a scoped edit while enforcing per-principal operations, memory, observation and queue limits.

## Negative and recovery cases

Foreign-region edits, allocation amplification and repeated rejected batches neither mutate nor leak quota.
