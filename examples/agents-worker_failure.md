...
Status: **planned specification; not implemented**. Work item: **PR-015 — Agent lifecycle and cancellation**.

Dependencies: PR-007, PR-011, PR-014. Validation lanes: cpu.
...
```sh
omniweft_examples --example agents.worker_failure --headless --seed 7 --verify --output artifacts/agents.worker_failure
```
