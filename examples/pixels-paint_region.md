...
Status: **planned specification; not implemented**. Work item: **PR-021 — Exact texture region edits**.

Dependencies: PR-016, PR-018. Validation lanes: cpu, gpu.
...
```sh
omniweft_examples --example pixels.paint_region --gpu --seed 7 --verify --output artifacts/pixels.paint_region
```
...
Write a known RGBA8 rectangle, verify exact bytes and untouched neighbors, then verify bounded GPU upload/readback.

## Negative and recovery cases

Reject invalid format, row pitch, bounds, stale revision and oversize writes without partial texture changes.
