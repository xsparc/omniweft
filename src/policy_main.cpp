...
ow::control::Config options(int argc, char* argv[]) {
  ...
  ow::control::Config result;result.max_slots=8;
  ...
  for (int index = 1; index < argc; index += 2) {
    ...
    else if (key == "--max-slots" && number == 8) result.max_slots = number;
...
class Owner {
 public:
  Owner(const control::Config& config,Time deadline,std::atomic<bool>& stop)
    ...
    auto presentation=std::make_shared<control::PresentationStatus>();presentation->enabled=false;
...
int main(int argc,char* argv[]) {
  try {
    ...
    LifetimeGuard lifetime(deadline+Milliseconds(1000));
    std::atomic<bool> stop=false;
    Owner owner(config,deadline,stop);
    std::jthread thread([&]{owner.run();});
    const auto result=control::run_policy(config,owner.callbacks());
    stop.store(true);owner.wake();thread.join();
    return owner.failed()?4:result;
