...
## PR-004 platform priority and private-host evidence

The maintainer's [PR-004 clarification](execution/AUTHORIZATION.md#rendering-verification-and-privacy-clarification) prioritizes physical Windows GPU verification on the current host, with conservative Linux Docker attempts. For this slice, Windows hardware checks are the immediate required target; Linux CPU/software/container results are identified separately and unavailable Linux GPU evidence remains `not_run`. The earlier two-OS rollout target is a future support goal, not a claim supplied by one Windows run. No Linux desktop or cross-platform driver support is claimed until its actual evidence exists.

Local raw evidence stays private. Use `python tools/export_public_evidence.py --help` to create a separate public export of baseline manifests: it verifies artifact hashes and clean candidate metadata, preserves assertion outcomes and counts, and copies only the exact known bootstrap result schema with new public-manifest hashes. Free-form strings and compound values are omitted; other artifact bytes remain private verified hash commitments. Workstation executables/PDBs, raw command output, host/account names, absolute machine paths and hardware identifiers are excluded. New render evidence uses an allowlisted schema before persistence. Publish only reviewed public exports; a redactable hash commitment is not a claim that omitted logs were published.

PR-005's candidate CPU lane exercises the actual native authenticated host and separate Python SDK with independent full-state, expiry, framing and mutation-proof checks. See [sdk.move_cube](../examples/sdk-move_cube.md) and [its handoff](execution/PR-005-HANDOFF.md) for actual current results; the plan alone is not evidence.
