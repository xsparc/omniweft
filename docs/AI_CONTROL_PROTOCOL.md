...
Status: PR-002 defines the [authoritative schema 0.1 envelope](../schemas/protocol-0.1.schema.json) and native structural parse/serialize API; see [its handoff](execution/PR-002-HANDOFF.md) for current verification and integration state. PR-003 adds `entity.delete` with required `child_policy: "reject_if_children"` and a synchronous trusted-host atomic execution boundary. PR-002-era readers reject that additive operation as `UNSUPPORTED_OPERATION`; all earlier envelope meanings remain unchanged. PR-005 adds three merged authenticated local endpoints and a separate typed Python SDK; its [frozen synchronous profile](execution/PR-005-PLAN.md), [wire schema](../schemas/control-0.1.schema.json) and [handoff](execution/PR-005-HANDOFF.md) record implemented scope, verification and merge. The fuller gateway, scheduling and retry behavior below remains planned. A `schema_valid` result alone does not authorize or apply a transaction.

PR-009 adds native-only `entity.reparent` with required target, nullable parent and preserve_world/preserve_local mode; see [objects.hierarchy](../examples/objects-hierarchy.md). Older native readers reject it. Existing remote control/policy profiles continue advertising only create/set/delete and reject the new operation before owner scheduling or sequence consumption. They also reject hierarchy snapshots, so no lossy format2 response reaches an unchanged SDK. This additive native schema support grants no remote capability.
